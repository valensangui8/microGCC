...
//    if (lookupSymbol(_context->symbolTable, *decl->identifier) != NULL) {
...
//        if (decl->declarationSuffix->functionSuffix->type == SUFFIX_BLOCK) {
...
//            }
//
//            addParameter(_context->symbolTable, *param->identifier, param->type, paramOffset);
...
//    if (funcDecl->dataType != TYPE_INT && funcDecl->dataType != TYPE_CHAR && !_context->functionHasReturn) {
//        // Note: In our subset, we only have int and char, both need return
//        if (!_context->functionHasReturn) {
...
//            return TYPE_INT; // Arithmetic operations return int
//        }
//
//        case EXPRESSION_NOT: {
